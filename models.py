...
class Student(db.Model):
   ...
   def __init__(self, name: str, number: str, age: int, _class: str) -> None:
      self.name = name
      self.number = number
      self.age = age
      self._class = _class
...
class Lessons(db.Model):
   ...
   def __init__(self, lesson_name: str, teacher_name: str) -> None:
...
class Grades(db.Model):
   ...
   def __init__(self, midterm: int, final: int, student_id: int, lesson_id: int) -> None:
...
class Logs(db.Model):
   ...
   def create_log(model: str, _type: str, *args: list[str]) -> str:
      ...
      # ANS: add note to student
         ...
         desc = f"Student ID {args[0]} and Lesson ID {args[1]} has been deleted from {model} table"
